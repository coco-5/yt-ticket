module.exports={
    ...
    // 插件
    plugins: [
        // 写入app.js指定位置，小程序源码app.js需要插入的位置写入： // @echo TEMP_PACK_INJECT
        function (content, {relative}) {
            if (!relative.match(/^\/app\.js$/)) return
            const text = `require('./uni-bootstrap.js');`
            process.env.TEMP_PACK_INJECT = text
            return content.replace(text, '')
        },
        // 为小程序源码注入环境变量，在需要用到环境变量的位置写入： // @echo TEMP_PACK_NODE_ENV
        function (content, {relative}) {
            process.env.TEMP_PACK_NODE_ENV = `var NODE_ENV='${process.env.NODE_ENV}';`
            return content
        },
        // 条件编译插件应该在混写插件之前使用
